      ...
      <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;">
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            ...
            <el-button v-if="$hasPermission('produseWaiting:produsewaiting:review')" type="text" size="small" @click="review(scope.row.id, 2)">审核通过</el-button>
            <el-button v-if="$hasPermission('produseWaiting:produsewaiting:review')" type="text" size="small" @click="review(scope.row.id, 3)">驳回</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
export default {
  ...
  methods: {
    ...
    review (id, status) {
      this.$confirm(this.$t('prompt.info', { 'handle': status === 2 ? '审核通过' : '审核驳回' }), this.$t('prompt.title'), {
