...
<!--        <el-form-item>
      ...
      <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;">
        ...
        <el-table-column prop="status" label="审核状态" header-align="center" align="center">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.status === 0" size="small" type="warning">未提交</el-tag>
            <el-tag v-if="scope.row.status === 1" size="small" type="primary">已提交审核</el-tag>
            <el-tag v-if="scope.row.status === 2" size="small" type="success">审核通过</el-tag>
            <el-tag v-if="scope.row.status === 3" size="small" type="danger">驳回</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            ...
            <el-button v-if="$hasPermission('produseWaiting:produsewaiting:update') && (scope.row.status === 0 || scope.row.status === 3)" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
            <el-button v-if="$hasPermission('produseWaiting:produsewaiting:delete') && (scope.row.status === 0 || scope.row.status === 3)" type="text" size="small" @click="deleteHandle(scope.row.id)">删除</el-button>
            <el-button v-if="$hasPermission('produseWaiting:produsewaiting:review') && (scope.row.status === 0 || scope.row.status === 3)" type="text" size="small" @click="review(scope.row.id)">提交审核</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
  ...
  methods: {
    ...
    review (id) {
      this.$confirm(this.$t('prompt.info', { 'handle': '提交审核' }), this.$t('prompt.title'), {
        ...
        this.$http.put(`/produseWaiting/produsewaiting/review`, {
          id: id,
          status: 1
        }).then(({ data: res }) => {
          if (res.code !== 0) {
