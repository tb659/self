<template>
  <el-dialog :visible.sync="visible" title="采购订单入库" :close-on-click-modal="false" :close-on-press-escape="false" width="90%">
    ...
    <el-table v-loading="dataListLoading" show-summary :summary-method="getSummaries" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column prop="warehouseName" fixed="right" show-overflow-tooltip label="入库库房" width="150" header-align="center" align="center">
        <template slot-scope="scope">
          <el-popover
            placement="right"
            width="300"
            ...
            <el-input v-model="scope.row.warehouseName" @focus="changLine(scope.row)" :readonly="true" slot="reference"></el-input>
            <el-button v-if="scope.row.warehouseName"   type="primary"  size="mini" @click="setAllWarehouseHandle(scope.row)">应用全部<i class="el-icon-document-copy el-icon--right"></i></el-button>
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column label="入库数量" fixed="right" header-align="center" align="center" width="150">
        <template slot-scope="scope">
          <div v-if="scope.row.status === '0'">
            <el-input-number controls-position="right" v-model="scope.row.putinQuantity" :max="scope.row.surplusQuantity" placeholder="入库数量" style="width: auto"></el-input-number>
          </div>
          <div v-if="scope.row.status === '1'">
            <el-input-number :disabled="true" controls-position="right" v-model="scope.row.putinQuantity" placeholder="入库数量" style="width: auto"></el-input-number>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="待入库数量" fixed="right" header-align="center" align="center" width="150">
        <template slot-scope="scope">
          <el-input-number controls-position="right" v-model="scope.row.surplusQuantity" :disabled="true" :min="0" placeholder="待入库数量" style="width: auto"></el-input-number>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    deptListTreeCurrentChangeHandle (data) {
      this.clickLine.warehouseId = data.id
      this.clickLine.warehouseName = data.name
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/waitinginstock/waitinginstock/unInStock/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.dataForm = {
        ...
        this.dataList.forEach(item => {
          item.putinQuantity = item.surplusQuantity
        })
      }).catch(() => {})
    },
    // 表单提交
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/waitinginstock/waitinginstock/waitinginstockIn', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
