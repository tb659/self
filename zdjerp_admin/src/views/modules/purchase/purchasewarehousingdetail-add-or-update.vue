...
export default {
  ...
  methods: {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/purchasewarehousing/purchasewarehousingdetail/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/purchasewarehousing/purchasewarehousingdetail/', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
