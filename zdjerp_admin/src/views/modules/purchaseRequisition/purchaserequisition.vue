  ...
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-demo__purchaserequisition}">
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        ...
        <el-form-item>
          <el-button v-if="$hasPermission('purchaserequisition:purchaserequisition:save')" type="primary" @click="addOrUpdateHandle()">{{ $t('add') }}</el-button>
        </el-form-item>
        <el-form-item>
          <el-button v-if="$hasPermission('purchaserequisition:purchaserequisition:delete')" type="danger" @click="deleteHandle()">{{ $t('deleteBatch') }}</el-button>
        </el-form-item>
      </el-form>
      <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;">
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            ...
            <el-button v-if="$hasPermission('purchaserequisition:purchaserequisition:update') && (scope.row.reviewStatus === 1 || scope.row.reviewStatus === 4)" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">{{ $t('update') }}</el-button>
            <el-button v-if="$hasPermission('purchaserequisition:purchaserequisition:delete') && (scope.row.reviewStatus === 1 || scope.row.reviewStatus === 4)" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
            <el-button v-if="$hasPermission('purchaserequisition:purchaserequisition:confirm') && (scope.row.reviewStatus === 1 || scope.row.reviewStatus === 4)" type="text" size="small" @click="confirmHandle(scope.row.id)">提交审核</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
  ...
  methods: {
    ...
    confirmHandle: function (id) {
      this.$confirm(this.$t('prompt.info', { 'handle': '提交审核' }), this.$t('prompt.title'), {
        ...
        this.$http.get(`/purchaserequisition/purchaserequisition/confirm/` + id).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
