<template>
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false"  :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" :label-width="$i18n.locale === 'en-US' ? '120px' : '110px'">
          <el-form-item label="保养内容名称" prop="maintainName">
          <el-input v-model="dataForm.maintainName" placeholder="保养内容名称" maxlength="200"></el-input>
      </el-form-item>
      <el-form-item label="类型" prop="maintainType">
      <el-select v-model="dataForm.maintainType" placeholder="请选择" style="width: 100%">
        <el-option
      ...
      <el-form-item prop="parentName" label="上级名称" class="dept-list" v-if="dataForm.maintainType==='3'">
        <el-popover v-model="deptListVisible" ref="deptListPopover" placement="bottom-start" trigger="click">
          <el-tree
            :data="deptList"
            :props="{ label: 'maintainName', children: 'children' }"
            node-key="id"
            ref="deptListTree"
...
export default {
  ...
  methods: {
    ...
    deptListTreeCurrentChangeHandle (data) {
      ...
      this.dataForm.parentName = data.maintainName
      this.deptListVisible = false
    },
    // 获取部门列表
    getDeptList () {
      return this.$http.get('/maintainMenu/maintainmenu/list', {
        params: {
          maintainType: 3,
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/maintainMenu/maintainmenu/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.dataForm = {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/maintainMenu/maintainmenu/', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
