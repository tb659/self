  ...
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-preproduction__preproductioninformation}">
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        ...
        <el-form-item style="width: 190px">
          <el-select v-model="dataForm.status" filterable clearable placeholder="当前工序" style="width: 100%;">
            <el-option label="未生产" :value="0"></el-option>
            <el-option label="钢筋制安" :value="1"></el-option>
            <el-option label="钢筋终检" :value="2"></el-option>
            <el-option label="钢筋入模及模板验收" :value="3"></el-option>
            <el-option label="埋件验收" :value="4"></el-option>
            <el-option label="混凝土浇筑" :value="5"></el-option>
            <el-option label="拆模外观验收" :value="6"></el-option>
            <el-option label="成品入库" :value="7"></el-option>
            <el-option label="成品出库装车" :value="8"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item>
          <el-button v-if="$hasPermission('preproduction:preproductioninformation:save')" type="primary" @click="addOrUpdateHandle()">{{ $t('add') }}</el-button>
        </el-form-item>
        <el-form-item>
          <el-button v-if="$hasPermission('preproduction:preproductioninformation:delete')" type="danger" @click="deleteHandle()">{{ $t('deleteBatch') }}</el-button>
        </el-form-item>
        <el-form-item>
          <el-button v-if="$hasPermission('preproduction:preproductioninformation:export')" type="info" @click="exportHandle()">下载导入模板</el-button>
        </el-form-item>
        <el-form-item>
          <el-upload
            class="upload-demo"
            ...
            <el-button v-if="$hasPermission('preproduction:preproductioninformation:import')" type="primary">导入</el-button>
          </el-upload>
        </el-form-item>
        <el-form-item style="width: 190px">
          <el-select v-model="printType" filterable clearable placeholder="批量打印类型" style="width: 100%;">
            ...
            <el-option label="新二维码" value="3"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          <el-button v-if="$hasPermission('preproduction:preproductioninformation:setBuildingAndFloorHandle')" type="primary" @click="setBuildingAndFloorHandle()">批量设置楼层</el-button>
        </el-form-item>
      </el-form>
      <el-table v-loading="dataListLoading" :data="dataList" :row-key="getRowKeys" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;">
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            ...
            <el-button v-if="$hasPermission('preproduction:preproductioninformation:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">{{ $t('update') }}</el-button>
            <el-button v-if="$hasPermission('preproduction:preproductioninformation:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
            <el-button v-if="$hasPermission('preproduction:preproductioninformation:printQrCode')" type="text" size="small" @click="printQrCode(scope.row)">二维码</el-button>
            <el-button v-if="$hasPermission('preproduction:preproductioninformation:printQrCode')" type="text" size="small" @click="printQrCodeSmall(scope.row)">二维码(小)</el-button>
            <el-button v-if="$hasPermission('preproduction:preproductioninformation:printQrCode')" type="text" size="small" @click="printNewQrCode(scope.row)">新二维码</el-button>
            <el-button v-if="$hasPermission('preproduction:preproductioninformation:printQrCode') && scope.row.printStatus === 1" type="text" size="small" @click="showPrintRecord(scope.row)">打印记录</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
      ...
      <set-building-and-floor v-if="setBuildingAndFloorVisible" ref="setBuildingAndFloor" @refreshDataList="getDataList"></set-building-and-floor>
    </div>
  </el-card>
</template>
...
import SetBuildingAndFloor from './setbuildingandfloor'
...
export default {
  ...
  components: {
    AddOrUpdate, ViewInfo, SetBuildingAndFloor
  },
  watch: {
    daterange (val) {
      if (val != null) {
  ...
  methods: {
    setBuildingAndFloorHandle () {
      if (this.dataListSelections.length <= 0) {
        return this.$message({
          message: '请选择需要批量设置楼层的构件',
          type: 'warning',
          duration: 3000
        })
      }
      this.setBuildingAndFloorVisible = true
      this.$nextTick(() => {
        this.$refs.setBuildingAndFloor.dataForm.ids = this.dataListSelections.map(item => item[this.mixinViewModuleOptions.deleteIsBatchKey])
        this.$refs.setBuildingAndFloor.init()
      })
    },
    // 上传之前
    ...
    batchPrint () {
      ...
      var dataListNew = this.dataListSelections.sort(function (a, b) {
        return a.systemCoding.localeCompare(b.systemCoding, 'zh-CN', { numeric: true })
      })
      var data = {}
      data.reqParam = '1'
      data.lang = 'zh'
      ...
      if (this.printType === '1') {
        dataListNew.forEach((item, index) => {
          ...
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '350,45,50,0,\'微软雅黑\',1,900,0,0,0,ID: ' + item.systemCoding + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,180,50,0,\'微软雅黑\',1,900,0,0,0,项目名称：' + item.productName + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,255,50,0,\'微软雅黑\',1,900,0,0,0,构件名称：' + item.componentName + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,330,50,0,\'微软雅黑\',1,900,0,0,0,外形尺寸：' + item.outlineSize + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,405,50,0,\'微软雅黑\',1,900,0,0,0,重量：' + item.amount + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,480,50,0,\'微软雅黑\',1,900,0,0,0,检验员盖章：' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,590,40,0,\'微软雅黑\',1,900,0,0,0,' + item.productionUnit + '' })
          printparamsJsonArray.push({ 'PTK_DrawBar2D_QR': '730,390,120,120,0,6,2,0,0,' + `${window.SITE_CONFIG[ 'printURL2' ]}` + '/products/products?informationId=' + item.id })
      ...
      if (this.printType === '2') {
        dataListNew.forEach((item, index) => {
          ...
          printparamsJsonArray.push({ 'PTK_SetLabelHeight': '690,8,0,false' })
          printparamsJsonArray.push({ 'PTK_SetLabelWidth': '1100' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '290,45,50,0,\'微软雅黑\',1,900,0,0,0,ID: ' + item.systemCoding + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,180,50,0,\'微软雅黑\',1,900,0,0,0,项目名称：' + item.productName + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,255,50,0,\'微软雅黑\',1,900,0,0,0,构件名称：' + item.componentName + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,330,50,0,\'微软雅黑\',1,900,0,0,0,外形尺寸：' + item.outlineSize + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,405,50,0,\'微软雅黑\',1,900,0,0,0,重量：' + item.amount + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,480,50,0,\'微软雅黑\',1,900,0,0,0,检验员盖章：' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,590,40,0,\'微软雅黑\',1,900,0,0,0,' + item.productionUnit + '' })
          printparamsJsonArray.push({ 'PTK_DrawBar2D_QR': '670,390,120,120,0,6,2,0,0,' + `${window.SITE_CONFIG[ 'printURL2' ]}` + '/products/products?informationId=' + item.id })
      ...
      if (this.printType === '3') {
        dataListNew.forEach((item, index) => {
          ...
          printparamsJsonArray.push({ 'PTK_AnyGraphicsPrint': '160,0,103903660,C:\\logo.jpg,0,180,160,0' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '350,45,50,0,\'微软雅黑\',1,900,0,0,0,ID: ' + item.systemCoding + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '350,110,50,0,\'微软雅黑\',1,900,0,0,0,楼号/层号：' + item.buildingNo + '/' + item.floorNo })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,180,50,0,\'微软雅黑\',1,900,0,0,0,项目名称：' + item.productName + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,255,50,0,\'微软雅黑\',1,900,0,0,0,构件名称：' + item.componentName + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,330,50,0,\'微软雅黑\',1,900,0,0,0,外形尺寸：' + item.outlineSize + '' })
          printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,405,50,0,\'微软雅黑\',1,900,0,0,0,重量：' + item.amount + '' })
    ...
    printQrCode (row) {
      ...
      printparamsJsonArray.push({ 'PTK_SetLabelWidth': '1100' })
      printparamsJsonArray.push({ 'PTK_AnyGraphicsPrint': '160,0,103903660,C:\\logo.jpg,0,180,160,0' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '350,45,50,0,\'微软雅黑\',1,900,0,0,0,ID: ' + row.systemCoding + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,180,50,0,\'微软雅黑\',1,900,0,0,0,项目名称：' + row.productName + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,255,50,0,\'微软雅黑\',1,900,0,0,0,构件名称：' + row.componentName + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,330,50,0,\'微软雅黑\',1,900,0,0,0,外形尺寸：' + row.outlineSize + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,405,50,0,\'微软雅黑\',1,900,0,0,0,重量：' + row.amount + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,480,50,0,\'微软雅黑\',1,900,0,0,0,检验员盖章：' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,590,40,0,\'微软雅黑\',1,900,0,0,0,' + row.productionUnit + '' })
    ...
    printQrCodeSmall (row) {
      ...
      printparamsJsonArray.push({ 'PTK_SetLabelHeight': '690,8,0,false' })
      printparamsJsonArray.push({ 'PTK_SetLabelWidth': '1100' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '290,45,50,0,\'微软雅黑\',1,900,0,0,0,ID: ' + row.systemCoding + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,180,50,0,\'微软雅黑\',1,900,0,0,0,项目名称：' + row.productName + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,255,50,0,\'微软雅黑\',1,900,0,0,0,构件名称：' + row.componentName + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,330,50,0,\'微软雅黑\',1,900,0,0,0,外形尺寸：' + row.outlineSize + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,405,50,0,\'微软雅黑\',1,900,0,0,0,重量：' + row.amount + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,480,50,0,\'微软雅黑\',1,900,0,0,0,检验员盖章：' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '100,590,40,0,\'微软雅黑\',1,900,0,0,0,' + row.productionUnit + '' })
    ...
    printNewQrCode (row) {
      ...
      printparamsJsonArray.push({ 'PTK_AnyGraphicsPrint': '160,0,103903660,C:\\logo.jpg,0,180,160,0' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '350,45,50,0,\'微软雅黑\',1,900,0,0,0,ID: ' + row.systemCoding + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '350,110,50,0,\'微软雅黑\',1,900,0,0,0,楼号/层号：' + row.buildingNo + '/' + row.floorNo })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,180,50,0,\'微软雅黑\',1,900,0,0,0,项目名称：' + row.productName + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,255,50,0,\'微软雅黑\',1,900,0,0,0,构件名称：' + row.componentName + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,330,50,0,\'微软雅黑\',1,900,0,0,0,外形尺寸：' + row.outlineSize + '' })
      printparamsJsonArray.push({ 'PTK_DrawText_TrueType': '160,405,50,0,\'微软雅黑\',1,900,0,0,0,重量：' + row.amount + '' })
