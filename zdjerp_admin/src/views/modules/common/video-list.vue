<template>
    <el-dialog :visible.sync="visible" title="列表" :close-on-click-modal="false" :append-to-body="true" width="60%">
...
import Upload from '../oss/oss-upload-video'
export default {
    ...
    submitHandle (row) {
      ...
      this.$message({
        ...
        onClose: () => {
          this.visible = false
          if (row) {
            this.$emit('getVideoInfo', row)
          } else {
            this.$emit('getVideoInfo', imgInfo)
          }
        }
      })
    }
