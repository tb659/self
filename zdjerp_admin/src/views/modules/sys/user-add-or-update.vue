  ...
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item prop="mobile" :label="$t('user.mobile')">
        <el-input v-model="dataForm.mobile" :placeholder="$t('user.mobile')" :maxlength = '11'></el-input>
      </el-form-item>
...
export default {
  ...
  computed: {
    dataRule () {
      ...
      var validateMobile = (rule, value, callback) => {
        ...
        this.checkMobileUnique(rule, value, callback)
      }
      return {
        username: [
  ...
  methods: {
    ...
    checkMobileUnique (rule, value, callback) {
      this.$http.get(`/sys/user/checkMobileUnique`, {
        params: {
          'mobile': value,
          'id': this.dataForm.id
        }
        ...
        if (data.code === 0) {
          if (data.data.length > 0) {
            callback(new Error('该手机号已经存在,请重新输入'))
          } else {
            callback()
          }
        } else {
