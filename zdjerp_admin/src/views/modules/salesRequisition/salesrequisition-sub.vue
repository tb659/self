<template>
  <el-dialog :visible.sync="visible" title="生成出库单" :close-on-click-modal="false" :close-on-press-escape="false" width="90%">
    ...
    <el-table  show-summary :summary-method="getSummaries" :data="dataList" @selection-change="dataListSelectionChangeHandle" border style="width: 100%;">
      ...
      <el-table-column fixed="right" prop="deliveryQuantity" show-overflow-tooltip label="出库数量" width="150" header-align="center" align="center">
        <template slot-scope="scope">
          <el-input-number v-model="scope.row.deliveryQuantity" controls-position="right" :min="0" :max="scope.row.remainingQuantity" placeholder="数量" style="width: auto"></el-input-number>
        </template>
      </el-table-column>
      <el-table-column fixed="right" prop="quantity" show-overflow-tooltip label="数量" width="150" header-align="center" align="center"></el-table-column>
      <el-table-column fixed="right" prop="remainingQuantity" show-overflow-tooltip label="剩余数量" width="150" header-align="center" align="center"></el-table-column>
    </el-table>
    <template slot="footer">
      <el-button @click="visible = false">{{ $t('cancel') }}</el-button>
      <el-button type="primary" @click="dataFormSubmitHandle()">立即生成待出库单</el-button>
    </template>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    checkBoxChecked (row, index) {
      if (row.generateRetrievalStatus === 1) {
        return false// 禁用
      } else {
        return true// 不禁用
      }
    ...
    dataFormSubmitHandle: debounce(function () {
      if (this.dataListSelections.length <= 0) {
        return this.$message({
          message: '请选择要生成待出库单的产品',
          type: 'warning',
          duration: 2000
        })
      }
      for (let i = 0; i < this.dataListSelections.length; i++) {
        if (this.dataListSelections[i].deliveryQuantity === 0 || this.dataListSelections[i].deliveryQuantity === '') {
          this.$message({
            message: '请输入对应构件的数量',
            type: 'warning',
            duration: 2000
      ...
      this.dataForm.salesRequisitionDetailList = this.dataListSelections
      this.$http.post(`/salesRequisition/salesrequisition/createSalesRequisitionSub`, this.dataForm).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.$message({
